...
from prnet.models.modules import MultiScaleFusion
from prnet.models.attention import MultiSizeAttentionModule
from prnet.utils import (
    get_prototype_features,
...
class PRNet(nn.Module):
    def __init__(
        ...
        encoder = timm.create_model(
            backbone, features_only=True, out_indices=layers, pretrained=True
        ).eval()
        self.encoder = encoder.to(device)

        featuremap_dims = dryrun_find_featuremap_dims(
            self.encoder, input_size, len(layers), device
        )
        ...
        self.attn_module1 = MultiSizeAttentionModule(in_channels, self.heights[:-1])

        self.up4_to_3 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True),
            nn.Conv2d(
                self.feature_dimensions[3],
    ...
    def forward(self, images: Tensor, proto_features: List[Tensor]):
        ...
        # attention modules
        features = self.attn_module1(cfeatures)
...
def dryrun_find_featuremap_dims(
    ...
    for i in range(num_layers):
        featuremap_dims.append(
            {
                "num_features": dryrun_features[i].shape[1],
                "resolution": dryrun_features[i].shape[2:],
            }
        )
