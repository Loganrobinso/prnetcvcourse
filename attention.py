...
class MultiSizeAttentionModule(nn.Module):
    def __init__(self, in_channels: List[int], heights: List[int], num_layers: int = 3):
        super().__init__()
...
class MultiSizeAttention(nn.Module):
    def __init__(self, in_channels: int, height: int):
        ...
        self.q_proj1 = nn.Conv1d(
            proj_channels1, proj_channels1, kernel_size=1, groups=proj_channels1
        )
        self.k_proj1 = nn.Conv1d(
            proj_channels1, proj_channels1, kernel_size=1, groups=proj_channels1
        )
        self.v_proj1 = nn.Conv1d(
            proj_channels1, proj_channels1, kernel_size=1, groups=proj_channels1
        )
        ...
        self.q_proj2 = nn.Conv1d(
            proj_channels2, proj_channels2, kernel_size=1, groups=proj_channels2
        )
        self.k_proj2 = nn.Conv1d(
            proj_channels2, proj_channels2, kernel_size=1, groups=proj_channels2
        )
        self.v_proj2 = nn.Conv1d(
            proj_channels2, proj_channels2, kernel_size=1, groups=proj_channels2
        )
        ...
        self.q_proj3 = nn.Conv1d(
            proj_channels3, proj_channels3, kernel_size=1, groups=proj_channels3
        )
        self.k_proj3 = nn.Conv1d(
            proj_channels3, proj_channels3, kernel_size=1, groups=proj_channels3
        )
        self.v_proj3 = nn.Conv1d(
            proj_channels3, proj_channels3, kernel_size=1, groups=proj_channels3
        )
        ...
        self.q_proj4 = nn.Conv1d(
            proj_channels4, proj_channels4, kernel_size=1, groups=proj_channels4
        )
        self.k_proj4 = nn.Conv1d(
            proj_channels4, proj_channels4, kernel_size=1, groups=proj_channels4
        )
        self.v_proj4 = nn.Conv1d(
            proj_channels4, proj_channels4, kernel_size=1, groups=proj_channels4
        )
    ...
    def patchify(self, x, patch_size=16, num_channels=128):
        ...
        patches = x.reshape(
            shape=(x.shape[0], num_channels, h, patch_size, w, patch_size)
        )
        patches = torch.einsum("nchpwq->nhwcpq", patches)
        patches = patches.reshape(
            shape=(x.shape[0], h * w, num_channels * patch_size**2)
        )
